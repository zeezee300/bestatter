...
export function createCommercialModule(ctx) {
	...
	function funeralScope() {
		const type = String(state.currentCase?.masterData?.funeral_type || state.currentCase?.funeralType || '').toLowerCase()
		if (type.includes('erd')) return 'BURIAL'
		if (['feuer', 'urne', 'see', 'baum'].some((value) => type.includes(value))) return 'CREMATION'
		return 'ALL'
	}
