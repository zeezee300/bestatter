...
export function createRecordsModule(ctx) {
	const { root, urls, state, recordLabels, apiBase } = ctx
	...
	function documentOutputRow(item) {
		...
		const fileId = pdfId || docxId
		const printablePdfId = pdfId || (format === 'PDF' || String(item.data?.mimeType || '').toLowerCase() === 'application/pdf' ? docxId : 0)
		const mailSource = String(pdfId ? (item.data?.pdf?.fileName || item.data?.pdf?.path || `${item.title}.pdf`) : (item.data?.fileName || path || item.title))
		const mailFileName = mailSource.split(/[\\/]/).pop() || (pdfId ? `${item.title}.pdf` : item.title)
		const mailMimeType = pdfId ? 'application/pdf' : String(item.data?.mimeType || '')
		...
		const caseNumber = recordCaseNumber(item)
		const actionData = `data-file-id="${fileId}" data-document-title="${esc(item.title)}" data-case-number="${esc(caseNumber)}" data-file-name="${esc(mailFileName)}" data-mime-type="${esc(mailMimeType)}" data-share-url="${esc(`${apiBase}/cases/${Number(item.caseId)}/files/${fileId}/content`)}"`
		const printData = printablePdfId ? `${actionData.replace(`data-file-id="${fileId}"`, `data-file-id="${printablePdfId}"`)} data-print-url="${esc(`${apiBase}/cases/${Number(item.caseId)}/files/${printablePdfId}/content.pdf`)}"` : ''
		return `<article class="bp-document-output" data-record-row="${item.id}">${previewUrl ? `<button class="bp-document-thumb preview-nextcloud-file" data-file-id="${previewId}" aria-label="Vorschau ${esc(item.title)}"><span class="bp-document-thumb-fallback" aria-hidden="true"><b>${esc(format)}</b><small>Vorschau öffnen</small></span><img src="${esc(previewUrl)}" alt="" loading="lazy"></button>` : `<div class="bp-document-thumb bp-document-thumb-empty"><b>${esc(format)}</b><small>Keine Vorschau</small></div>`}<div class="bp-record-content"><b>${caseNumber ? `<span class="bp-case-number">Fall ${esc(caseNumber)}</span>` : ''}${esc(item.title)}</b><small>${esc(item.data?.documentType || 'Allgemein')} · ${esc(item.status)} · ${esc(displayDate)}</small><small class="bp-document-location">Ablage: ${esc(item.data?.path || 'Fallakte')}</small>${item.updatedBy || item.createdBy ? `<small>Bearbeitet von: ${esc(item.updatedBy || item.createdBy)}</small>` : ''}${immutable ? '<small>Finale Ausgabe – schreibgeschützt.</small>' : (editable ? '<small>Arbeitsstand – kann in Nextcloud bearbeitet werden.</small>' : '<small>Technische oder schreibgeschützte Datei.</small>')}${item.data?.paperSignature ? `<small>${item.data.paperSignature.source === 'GENERATED' ? 'Finale PDF-Revision' : 'Separater Vertrag'} · ${item.status === 'SIGNATURE_REVIEW' ? 'Sichtprüfung ausstehend' : 'handschriftlich unterschrieben'}</small>` : ''}${item.data?.pdfWarning ? `<small class="bp-error">${esc(item.data.pdfWarning)}</small>` : ''}</div><div class="bp-row-actions">${item.status === 'SIGNATURE_REVIEW' && item.data?.paperSignature && Number(item.id) > 0 ? `<button class="bp-primary review-paper-contract" data-record-id="${Number(item.id)}">Unterschriften prüfen</button>` : ''}${editable && docxId ? `<button class="bp-secondary open-nextcloud-file" data-file-id="${docxId}">In Nextcloud bearbeiten</button>` : ''}${pdfId ? `<button class="bp-primary open-nextcloud-file" data-file-id="${pdfId}">PDF öffnen</button>` : ''}${!pdfId && docxId ? `<button class="bp-secondary open-nextcloud-file" data-file-id="${docxId}">${editable?'Dokument öffnen':'Datei öffnen'}</button>` : ''}${printablePdfId ? printButton('print-nextcloud-pdf', 'Drucken', printData, 'PDF direkt drucken') : ''}${fileId ? mailButton('mail-nextcloud-file', 'Mit Mailprogramm teilen', actionData, 'Datei an ein lokales Mailprogramm übergeben') : ''}</div></article>`
	}
